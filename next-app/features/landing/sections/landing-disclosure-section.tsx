    ...
    <section id="selective-disclosure" className="relative px-6 py-24 sm:py-32 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-20 grid gap-8 lg:grid-cols-2 lg:items-stretch">
          ...
          <div className="relative flex flex-col justify-between rounded-[3rem] border border-foreground/5 bg-neutral-900/40 p-10 backdrop-blur-3xl shadow-2xl">
            <div className="space-y-8">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-foreground/5 text-primary shadow-[inset_0_0_20px_rgba(var(--primary-rgb),0.2)]">
              ...
              <div className="space-y-4">
                <h3 className="text-4xl font-bold tracking-tight text-foreground leading-tight">
                  ...
                  <span className="text-neutral-600 italic">completely</span> dark.
                </h3>
            ...
            <div className="mt-12 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {["Payer Identity", "Full Balance", "Wallet History", "Timestamp Metadata"].map((item) => (
                  <div key={item} className="rounded-full border border-foreground/5 bg-black/40 px-4 py-2 text-[11px] font-medium text-neutral-400">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col gap-4">
            <p className="px-4 text-sm text-neutral-600 mb-2">Available Disclosures</p>
            {proofTypes.map((proof, index) => (
              <div 
                key={proof.title} 
                className="group relative flex items-center gap-6 rounded-full  border p-4 transition-all duration-500  hover:bg-foreground/2 hover:translate-x-2"
              >
           

                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-foreground/5 text-primary transition-all group-hover:border-primary/20 group-hover:text-primary group-hover:shadow-[0_0_20px_rgba(var(--primary-rgb),0.1)]">
                  <ShieldCheck className="h-5 w-5" />
                </div>
                
                <div className="space-y-1">
                  <h3 className="text-lg font-bold text-foreground tracking-tight">{proof.title}</h3>
                  <p className="text-sm text-neutral-500 group-hover:text-neutral-400">{proof.body}</p>
                </div>
            ...
            <div className="mt-4 rounded-[2rem] border border-dashed border-foreground/10 p-6">
